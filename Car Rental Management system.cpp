...
struct CustomerInfo {
    ...
    string feedback;
	double paid;
    double final;      // Tracks the current remaining amount due
    double totalBill;  // Stores the original invoice amount (Initialize to 0 when creating customer)
    double totalPaid;
...
// Admin Login Function
    ...
    for (int attempts = 3; attempts > 0; attempts--) {
        ...
        cout << "\t\t\t\t\t   ********* Remaining Attempts "<< attempts - 1  <<" *********" << "\n\n";
    }
...
// method for Customer login 
   bool commonLogin() {
    ...
    for (int attempts = 3; attempts > 0; attempts--) {
        ...
        if (regname != loginname && regpass != loginpass)
            cout << " You Entered The Wrong Username and Password!!" << endl;
        ...
        if (attempts > 1)
            cout << "\n\t\t\t\t\t   ********* Remaining Attempts "
                 << attempts - 1 << " *********" << endl << endl;
    }

    cout << "\n Sorry, You Have No More Attempts !!" << endl;
    return false;
}
// destructor
	~User();
...
// method to Add Payment
    void addPayment() {
	    ...
	    // 1. Initialize totalBill on the very first installment
	    if (CustomerInfo::info.totalBill == 0) {
	        CustomerInfo::info.totalBill = CustomerInfo::info.final;
	        CustomerInfo::info.totalPaid = 0; 
	        CustomerInfo::info.installmentHistory = ""; // Ensure history starts empty
	    }
	
	    // 2. Display payment history breakdown on screen
	    cout << " Total Amount of Bill: " << CustomerInfo::info.totalBill << endl;
	    if (CustomerInfo::info.totalPaid > 0) {
	        cout << " Total Amount Previously Paid: " << CustomerInfo::info.totalPaid << endl;
	        cout << " Past Installments: [" << CustomerInfo::info.installmentHistory << "]" << endl;
	    }
	    cout << " Current Remaining Amount Due: " << CustomerInfo::info.final << endl;
	    cout << "----------------------------------------------------" << endl;
	
	    // 3. Stop processing if already paid off
	    if (CustomerInfo::info.final <= 0) {
	        cout << " This bill has already been fully paid!" << endl;
	        cout << " Enter any key to return to the main menu.";
	        getch();
	        system("cls");
	        return;
	    ...
	    // 4. Read input directly into CustomerInfo::info.paid
	    cout << " Enter Amount To Pay Now: ";
	    cin >> CustomerInfo::info.paid;
	    
	    // Append the current payment into the history string record
	    if (CustomerInfo::info.installmentHistory == "") {
	        CustomerInfo::info.installmentHistory = to_string((int)CustomerInfo::info.paid);
	    } else {
	        CustomerInfo::info.installmentHistory += ", " + to_string((int)CustomerInfo::info.paid);
	    }
	    
	    // Update our historical tracking variable
	    CustomerInfo::info.totalPaid += CustomerInfo::info.paid;
	
	    // 5. Calculate logic based on CustomerInfo::info.paid
	    if (CustomerInfo::info.paid >= CustomerInfo::info.final) {
	        double change = CustomerInfo::info.paid - CustomerInfo::info.final;
	        cout << "\n Payment successful!" << endl;
	        if (change > 0) {
	            cout << " Cashback / Change: " << change << endl;
	        }
	        CustomerInfo::info.final = 0; // Fully cleared
	    } else {
	        CustomerInfo::info.final -= CustomerInfo::info.paid; // Deduct installment
	        cout << "\n Partial payment accepted." << endl;
	        cout << " New Remaining Amount: " << CustomerInfo::info.final << endl;
	    }
...
// Save Records
	void saveRecords()
	{
        ...
        // file handling to save payment receipt
	    ...
	    if (bill.is_open()) {
	        ...
	        bill << endl << "\t\t\t\t\t---------- Payment Transaction Details ----------" << endl << endl;
	        bill << " Total Base Rent: " << total << endl;
	        bill << " Discount Allowed: " << discount << endl;
	        bill << "----------------------------------------------------" << endl;
	        bill << " Original Total Bill: " << CustomerInfo::info.totalBill << endl;
	        bill << " Current Payment Made Now: " << CustomerInfo::info.paid << endl; // Perfectly matches the UI input
	        bill << " All Installments Paid: [ " << CustomerInfo::info.installmentHistory << " ]" << endl; // <-- NEW LINE ADDED
	        bill << " Total Accumulated Paid: " << CustomerInfo::info.totalPaid << endl;
	        bill << " Current Remaining Balance: " << CustomerInfo::info.final << endl;
	        bill << "----------------------------------------------------" << endl << endl;
